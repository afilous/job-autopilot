...
const TITLE_HARD_DISQUALIFIERS = [
  ...
  // FIX: bare "engineer" catch-all. TITLE_EXACT_TARGETS' 'special projects'
  // match was scoring "Special Projects Engineer" and "Systems Test
  // Engineer, Weather and Special Projects" at 90+ despite both being
  // genuine engineering ICs -- confirmed from real scored results tonight.
  // This supersedes the specific compound checks above (kept for clarity/
  // no regression risk) and catches any other engineer-titled role that
  // happens to contain an otherwise-matching S&O keyword.
  'engineer',
...
// FIX: normalizeLocation() appears to flag ANY string containing "remote"
// as isRemote: true, regardless of region qualifier -- confirmed from real
// scored jobs: Zscaler "Remote - UK" and "Remote - Ireland" both scored 90,
// and an Airwallex Bangalore-based role also passed the gate at 90.
// Defensive re-check inside scoreJob() below, since location.js itself
// isn't visible/editable in this session.
const NON_US_REGION_MARKERS = [
  'uk', 'united kingdom', 'ireland', 'canada', 'india', 'bangalore',
  'europe', 'emea', 'apac', 'latam', 'australia', 'singapore',
  'germany', 'france', 'spain', 'italy', 'netherlands', 'poland',
  'mexico', 'brazil', 'japan', 'china', 'philippines', 'vietnam',
];
...
function scoreJob({ title, department, description, location, company }, opts = {}) {
  ...
  // FIX: see NON_US_REGION_MARKERS comment above.
  const rawLoc = (location || '').toLowerCase();
  if (NON_US_REGION_MARKERS.some(r => rawLoc.includes(r))) return 0;
